...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        # Iterating through each value in the order domain values
        for value in assignment[var]:
            assignment_copy = copy.deepcopy(assignment)
            assignment_copy[var] = value

            if self.inference(assignment_copy, self.get_all_arcs()):
                # Inference was not a failure
                result = self.backtrack(assignment_copy)

                if result:
                    # Result was not a failure
                    return result
    ...
    def select_unassigned_variable(self, assignment):
        ...
        """
        # Here I'm selecting the variable with the least amount of values
    ...
    def inference(self, assignment, queue):
        ...
        while queue:
            ...
            if self.revise(assignment, x_i, x_j):
                # Returning false if the size of the domain is 0
                if not assignment[x_i]:
                    return False
                for x_k, _ in self.get_all_neighboring_arcs(x_i):
                    # Don't want to add old arcs
                    if x_k != x_j:
    ...
    def revise(self, assignment, i, j):
        ...
        for x in assignment[i]:
            ...
            between x_i and x_j then we'll delete x from D_i
            """
            if not sum(
                arc in self.constraints[i][j]
                for arc in list(self.get_all_possible_pairs([x], assignment[j]))
            ):
...
def create_sudoku_csp(filename):
    ...
    """
    csp = CSP()
    board = list(map(lambda x: x.strip(), open(f"puzzles/{filename}", "r")))
